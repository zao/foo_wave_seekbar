...
static void lazy_recording_init(lock_guard<uv_mutex_t> const&)
{
	if (!initialized) {
...
{
	if (!initialized) {
		lock_guard<uv_mutex_t> lock(logging_mutex);
