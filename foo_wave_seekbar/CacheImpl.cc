...
cache_impl::create_query(playable_location const& loc,
...
void
cache_impl::get_waveform(service_ptr_t<waveform_query> request)
{
...
void
cache_impl::remove_dead_waveforms()
{
    if (store) {
        defer_action([this] { store->remove_dead(); });
    }
}

void
cache_impl::compact_storage()
{
    if (store) {
        defer_action([this] { store->compact(); });
    }
}

void
cache_impl::rescan_waveforms()
{
    if (store) {
        defer_action([this] {
...
void
cache_impl::defer_action(std::function<void()> fun)
{
...
bool
cache_impl::is_location_forbidden(playable_location const& loc)
{
    return is_of_forbidden_protocol(loc);
}

bool
cache_impl::has_waveform(playable_location const& loc)
{
    if (store) {
...
void
cache_impl::remove_waveform(playable_location const& loc)
{
    if (store) {
        store->remove(loc);
    }
...
void
cache_impl::worker_main(size_t i, size_t n)
{
    uint64_t time_frequency;
    QueryPerformanceFrequency((LARGE_INTEGER*)&time_frequency);
    {
        std::string name = fmt::format("wave-processing-{}/{}", i + 1, n);
    ...
    while (1) {
        {
            ...
            for (size_t i = 0; i < 3; ++i) {
                ...
                if (!requests_by_urgency[i].empty()) {
            ...
            if (q.is_valid()) {
                ...
                switch (res) {
                    case process_result::not_done: {
                        // TODO(zao): Get progress from state, persist into
                        // query.
                        done = false;
