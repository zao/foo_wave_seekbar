...
void
throw_if_aborting(abort_callback const& cb)
{
    if (cb.is_aborting())
...
struct scoped_timer
{
    ...
    ~scoped_timer()
    {
        ...
        FB2K_console_formatter() << "Scan took " << t << " seconds.";
    }
};

bool
try_determine_song_parameters(service_ptr_t<input_decoder>& decoder,
                              t_uint32 subsong,
                              t_int64& sample_rate,
...
void
transpose(C1& out, C2 const& in, size_t width, int valid_input_rows)
{
    int in_rows = (int)(in.get_size() / width), in_cols = (int)width;
...
struct waveform_builder : analysis_pass
{
    ...
    bool valid_bucket() const { return bucket < bucket_count; }
    ...
    bool bucket_boundary() const { return samples_processed == bucket_ends(); }
    ...
    void process(audio_sample const* data, t_int64 frames)
    {
        for (unsigned k = 0; k < frames * channel_count; ++k) {
            ...
            float& min = minimum[target_offset];
            float& max = maximum[target_offset];
    ...
    ref_ptr<waveform> finalize_waveform() const
    {
        ...
        auto rms = this->rms;
        if (should_downmix) {
            auto downmix_one = [this](auto const* l) -> auto {
...
void
destroy_process_state(process_state* state)
{
    delete state;
}
            ...
            if (!state->builder->finished()) {
                ...
                if (store)
                    ...
                    FB2K_console_formatter()
                      << "Wave cache: could not open backend database, losing new data for " << loc;
...
ref_ptr<waveform>
cache_impl::render_waveform(process_state* state)
{
    return state->wf;
}

bool
cache_impl::is_refresh_due(process_state* state)
{
