...
namespace wave
{
  namespace direct3d9
  {
    ...
    effect_compiler_impl::effect_compiler_impl(d3d9_api const& api, CComPtr<IDirect3DDevice9> dev)
      : api(api), dev(dev)
    {
    }
...
#endif
        hr = api.D3DXCreateEffect(dev, &fx_body[0], fx_body.size(), nullptr, nullptr, flags, nullptr, &fx, &err);
        if (FAILED(hr))
        {
