...
namespace wave
{
	...
	private:
		...
		void open_store();
		void load_data();
		process_result::type process_file(service_ptr_t<waveform_query> q, std::unique_ptr<process_state>& state);
		ref_ptr<waveform> render_waveform(std::unique_ptr<process_state> const& state);
