...
namespace wave {
namespace direct3d9 {
CComPtr<IDirect3DTexture9>
frontend_impl::create_waveform_texture()
{
    CComPtr<IDirect3DTexture9> tex;
    dev->CreateTexture(
      2048, 1, mip_count, 0, texture_format, D3DPOOL_MANAGED, &tex, 0);
    return tex;
}
...
const GUID guid_fx_string = {
    ...
    0x2cff,
    0x4c59,
    { 0x81, 0xad, 0xe, 0xf2, 0x78, 0x4e, 0x9d, 0xf }
};
...
frontend_impl::create_default_resources()
{
    ...
    auto build_effect = [compiler](std::string body) -> ref_ptr<effect_handle> {
        ...
        bool success = compiler->compile_fragment(
          fx, diagnostic_collector(errors), body.c_str(), body.size());

        if (!success) {
            // TODO: propagate errors to parent DLL
            // console::formatter() << "Seekbar: Direct3D: effect compile
            // failed."; std::string text = simple_diagnostic_format(errors);
    ...
    // Compile current stored effect
    {
        ...
        if (conf.get_configuration_string(guid_fx_string,
                                          std_string_sink(stored_body))) {
            ref_ptr<effect_handle> fx;
