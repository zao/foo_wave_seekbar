...
namespace wave {
void
enqueue_desired(playable_location const& location)
{
    static_api_ptr_t<cache> c;
    auto q = c->create_query(location,
                             waveform_query::desired_urgency,
                             waveform_query::unforced_query);
    c->get_waveform(q);
}

void
seekbar_window::on_waveform(ref_ptr<waveform> wf)
{
...
void
seekbar_window::on_time(double t)
{
    if (t == 0.0)
        set_playback_time(t);
}
...
void
seekbar_window::on_duration(double t)
{
    std::unique_lock<std::recursive_mutex> lk(fe->mutex);
...
void
seekbar_window::on_location(playable_location const& loc)
{
    std::unique_lock<std::recursive_mutex> lk(fe->mutex);
    {
        ...
        meta->get_info_ref(info_ptr);
        replaygain_info rg = info_ptr->info().get_replaygain();
#define SET_REPLAYGAIN(Name)                                                   \
    fe->callback->set_replaygain(visual_frontend_callback::replaygain_##Name,  \
                                 rg.m_##Name);
...
static const GUID order_default = {
    0xbfc61179,
    0x49ad,
    0x4e95,
    { 0x8d, 0x60, 0xa2, 0x27, 0x06, 0x48, 0x55, 0x05 }
};
static const GUID order_repeat_playlist = {
    0x681cc6ea,
    0x60ae,
    0x4bf9,
    { 0x91, 0x3b, 0xbb, 0x5f, 0x4e, 0x86, 0x4f, 0x2a }
};
static const GUID order_repeat_track = {
    0x4bf4b280,
    0x0bb4,
    0x4dd0,
    { 0x8e, 0x84, 0x37, 0xc3, 0x20, 0x9c, 0x3d, 0xa2 }
};
static const GUID order_random = {
    0x611af974,
    0x4316,
    0x43ac,
    { 0xab, 0xec, 0xc2, 0xea, 0xa3, 0x5c, 0x3f, 0x9b }
};
static const GUID order_shuffle_tracks = {
    0xc5cf4a57,
    0x8c01,
    0x480c,
    { 0xb3, 0x34, 0x36, 0x19, 0x64, 0x5a, 0xda, 0x8b }
};
static const GUID order_shuffle_albums = {
    0x499e0b08,
    0xc887,
    0x48c1,
    { 0x9c, 0xca, 0x27, 0x37, 0x7c, 0x8b, 0xfd, 0x30 }
};
static const GUID order_shuffle_folders = {
    0x83c37600,
    0xd725,
    0x4727,
...
void
seekbar_window::test_playback_order(t_size playback_order_index)
{
    ...
    if (current_order == order_default ||
        current_order == order_repeat_playlist ||
        current_order == order_shuffle_albums ||
        current_order == order_shuffle_folders) {
        t_size playlist, index;
        if (pm->get_playing_item_location(&playlist, &index)) {
            t_size count = pm->playlist_get_item_count(playlist);
            metadb_handle_ptr next =
              pm->playlist_get_item_handle(playlist, (index + 1) % count);
            try {
