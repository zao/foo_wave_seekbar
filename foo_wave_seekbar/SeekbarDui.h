  ...
  , seekbar_window
{
    ...
    static void g_get_name(pfc::string_base& out) { out = "Waveform Seekbar"; }
    ...
    void notify(GUID const& what,
                t_size param1,
                void const* param2,
                t_size param2Size);
    seekbar_dui(ui_element_config::ptr cfg,
                ui_element_instance_callback::ptr callback);
    ~seekbar_dui();

  protected:
    virtual void get_colors();
    virtual bool forward_rightclick()
    {
        return callback->is_edit_mode_enabled();
    }
