...
image_cache::image_cache(d2d_api const& api)
  : api(api)
...
bool
worker_task_ready(image_cache* self)
{
...
direct2d1_frontend::direct2d1_frontend(HWND wnd,
                                       wave::size size,
  ...
  , last_serial_issued(0)
{
  ...
  if (!api.d2d_module) {
    throw std::runtime_error(
      "Direct2D DLLs not found. Ensure you're running Vista SP2 or later with "
...
void
direct2d1_frontend::clear()
{}

void
direct2d1_frontend::draw()
{
  ...
  while (!SUCCEEDED(hr)) {
    ...
    if (callback.get_shade_played()) {
      ...
      if (flip)
	rt->FillRectangle(D2D1::RectF(play_x, 0, size.width, size.height),
	                  overlay_brush);
      else
...
void
direct2d1_frontend::present()
{}

void
direct2d1_frontend::update_size()
{
                                           ...
                                           wave::size size)
{
  ...
  switch (callback.get_downmix_display()) {
    case config::downmix_mono:
      if (wf->get_channel_count() > 1)
	wf = downmix_waveform(wf, 1);
      break;
    case config::downmix_stereo:
      if (wf->get_channel_count() > 2)
	wf = downmix_waveform(wf, 2);
      break;
  }
...
void
direct2d1_frontend::on_state_changed(state s)
{
...
void
image_cache::update_texture_target(ref_ptr<waveform> wf,
                                   pfc::list_t<channel_info> infos,
                                   ...
                                   uint64_t serial)
{
  ...
  infos.enumerate([&channel_indices,
                   channel_numbers](channel_info const& info) {
    if (info.enabled) {
      auto I =
        std::find(channel_numbers.begin(), channel_numbers.end(), info.channel);
  ...
  while (!SUCCEEDED(hr)) {
    ...
    rt->Clear(color_to_d2d1_color(colors.background));
    int x = 0;
    wave_geometries.enumerate([this, &brushes, rt, size, &x, index_count](
                                pfc::com_ptr_t<ID2D1PathGeometry> const& geom) {
...
template<typename T>
std::shared_ptr<pfc::list_t<T>>
copy_list(const pfc::list_base_const_t<T>& l)
{
...
void
direct2d1_frontend::update_data()
{
  ref_ptr<waveform> wf;
  if (!callback.get_waveform(wf))
    wf = make_placeholder_waveform();
...
void
direct2d1_frontend::regenerate_brushes()
{
  palette p = { callback.get_color(config::color_background),
