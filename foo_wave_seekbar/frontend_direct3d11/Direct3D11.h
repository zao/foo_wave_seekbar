...
struct effect_parameters
{
    ...
    void assign(attribute& dst, DirectX::SimpleMath::Vector4 v)
    {
        dst.kind = attribute::VECTOR4;
        memcpy(dst.v.data(), &v, sizeof(float) * 4);
    }
    void assign(attribute& dst, DirectX::SimpleMath::Matrix m)
    {
...
struct frontend_impl : visual_frontend
{
  ...
  private: // Misc state
    ...
    CComPtr<ID3D11RenderTargetView> back_buffer_rt;

    using TexWithSrv = std::pair<CComPtr<ID3D11Texture2D>, CComPtr<ID3D11ShaderResourceView>>;
    std::map<unsigned, TexWithSrv> channel_textures;
...
struct config_dialog : CDialogImpl<config_dialog>
{
  ...
  private:
    ...
    struct scintilla_func
    {
        ...
        template<class A1>
        sptr_t send(unsigned int cmd, A1 a1) const
        {
            return send(cmd, (uptr_t)a1);
        }

        template<class A1, class A2>
        sptr_t send(unsigned int cmd, A1 a1, A2 a2) const
        {
            return send(cmd, (uptr_t)a1, (sptr_t)a2);
        }
